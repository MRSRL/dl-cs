        ...
        for b in wrap(range(num_batches)):
            ...
            if x_end != x_act_end:
                ...
                kspace_input_batch = np.concatenate(
                    (kspace_input_batch,
                     np.zeros((pad,) + kspace_input_batch.shape[1:], np.complex64)), axis=0)
                sensemap_input_batch = np.concatenate(
                    (sensemap_input_batch,
                     np.zeros((pad,) + sensemap_input_batch.shape[1:], np.complex64)), axis=0)
            feed_dict = {self.tf_kspace_input: kspace_input_batch,
                         self.tf_sensemap_input: sensemap_input_batch}
            out = self.tf_sess.run([self.tf_kspace_output], feed_dict=feed_dict)[0]
            kspace_output[x_start:x_act_end, :, :, :] = out

            if self.debug_plot:
                imout = mri.sumofsq(fftc.ifftc(fftc.ifftc(out, axis=1), axis=2), axis=-1)
                image_output[x_start:x_act_end, :, :] = imout

                image_axial_disp = np.concatenate(
                    (image_input[x_start, :, :],
                     image_output[x_start, :, :]), axis=1)
                image_sag_disp = np.concatenate(
                    (image_input[:, :, image_input.shape[-1]//2],
                     image_output[:, :, image_output.shape[-1]//2]), axis=1)
                pyplot.figure(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('kspace_input', action='store',
    ...
    model = DeepRecon(args.model_dir, num_channels, shape_z, shape_y,
