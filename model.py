...
def _conv2d(tf_input,
    ...
    if circular and pad > 0:
        with tf.name_scope('circular_crop'):
            if data_format == 'channels_last':
                tf_output = tf_output[:, pad:(shape_z + pad), pad:(
                    shape_y + pad), :]
            else:
                tf_output = tf_output[:, :, pad:(shape_z + pad), pad:(
                    shape_y + pad)]
...
def _res_block(net_input,
    ...
    with tf.name_scope(name):
        ...
        if circular:
            with tf.name_scope('circular_crop'):
                if data_format == 'channels_last':
                    net_cur = net_cur[:, pad:(pad + shape_z), pad:(
                        pad + shape_y), :]
                else:
                    net_cur = net_cur[:, :, pad:(pad + shape_z), pad:(
                        pad + shape_y)]
        ...
        if circular:
            with tf.name_scope('circular_crop'):
                if data_format == 'channels_last':
                    ...
                    net_dense = net_dense[:, pad:(pad + shape_z), pad:(
                        pad + shape_y), :]
                else:
                    net = net[:, :, pad:(pad + shape_z), pad:(pad + shape_y)]
                    net_dense = net_dense[:, :, pad:(pad + shape_z), pad:(
...
def unrolled_prox(ks_input,
    ...
    with tf.variable_scope(scope):
        ...
        for i_step in range(num_grad_steps):
            ...
            scope_name = 'iter' if resblock_share else iter_name
            ...
            with tf.variable_scope(
                ...
                with tf.variable_scope('prox'):
                    ...
                    # Change to channels_first for faster compute on GPU
                    ...
                    if do_dense:
                        if im_dense is not None:
                            im_dense = tf.concat([im_dense, im_dense_k],
                                                 axis=1)
                        else:
                            im_dense = im_dense_k
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        for _ in range(num_blocks):
            x = _res_block(
                ...
                kernel_size=kernel_size,
                data_format=data_format_b,
                circular=True,
                batchnorm=batchnorm)
            # 1x1 convolutions with strides to reduce image size and increase
            # features
            num_features_b *= 2
            x = _batch_norm_relu(
                x,
